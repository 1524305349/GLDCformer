...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        ...
                        if isinstance(ret, tuple):
                            outputs = ret[0]
                            # 获取 mask rate (aux_loss)
                            if isinstance(ret[1], torch.Tensor) and ret[1].ndim <= 1:
                                aux_loss = ret[1]
                            else:
                                aux_loss = 0
                        else:
                        ...
                        if isinstance(aux_loss, torch.Tensor):
                            epoch_mask_rates.append(aux_loss.item())
                        elif isinstance(aux_loss, float):
                            epoch_mask_rates.append(aux_loss)
            ...
            if len(epoch_mask_rates) > 0:
                avg_keep = np.mean(epoch_mask_rates)
                print(">> Epoch: {} Summary | Avg Keep Ratio: {:.2%} | Avg Masked Ratio: {:.2%}".format(
                    epoch + 1, avg_keep, 1.0 - avg_keep))
